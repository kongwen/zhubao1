...
require(['jquery', "domReady!"], function($) {
    var postEle = function(){
        ...
        *
        */
        var standard = 250,wRow = 0,rCount= 0,top= 0,imgArr = [],left= 0,lastLen= 0,lastPerW,lastTop = 0,lastPer=1;
                ...
                imgArr.push({w: $(e).width() * (standard / $(e).height())});
                 if (wRow > 910) {
                     ...
                     $.each(imgArr, function (j, ths) {
                         ...
                         $('section ul li').eq((rCount + j)).animate({left: left + "px", top: top + "px"}, 500);
                         left += ths.w * per + 5;
                     });
        ...
        if($('section ul li').length-rCount){
            ...
            if(wHRow < 910/3 *2){
                ...
                for(var i = 0;i< syu ;i++ ){
                    ...
                    if(i < lastLen) thsMW =  thsMW / lastPer;
    ...
    var byPostEle =function(arrImg){
        //冒泡排序
        for(var i = 0;i < arrImg.length;i++){
            for (var j = 0; j < arrImg.length -i -1; j++) {
                if(arrImg[j].h >  arrImg[j+1].h){
                    var tempObj = arrImg[j];
                    arrImg[j] = arrImg[j+1];
                    arrImg[j + 1] = tempObj;
                }
            }
        ...
        var $html = ""
        for(var i = 0;i < arrImg.length;i++){
            $html += '<li style="height: 250px;width: '+ this.width * (250 / this.height) +'px"><img src="'+ arrImg[i].src +'"alt="' + arrImg[i].alt +'"/></li>';
        }
        $('#arrDate').append($html);
        postEle($html);
    }
            ...
            success: function(msg){
              ...
              $.each(msg, function (i, ths) {
                  $("<img/>").attr("src", ths.src).load(function() {
                      count ++;
                      var temp = {};
                      temp.w = this.width; temp.h = this.height;
                      temp.src = ths.src;temp.alt = ths.alt;
                      arrImg.push(temp);
                      if(count === dateInfoNum )  byPostEle(arrImg);
                  }).error(function(){
                      count ++;
                      if(count === dateInfoNum )  byPostEle(arrImg);
                  });
